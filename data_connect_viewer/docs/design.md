...
- 元 JSON 全文は必ず保存する
- `hostname` が既存レコードと一致する場合は新規登録せず更新する
- 更新時の項目反映ルールは、項目単位で変更しやすい構成とする
- `ipaddress`、`area`、`building`、`category`、`model` は既存値が空の場合のみ受信値を反映する
- `ping_test_result`、`exec_result` は既存値に関係なく受信値で上書きする
- 保存時に受信日時をシステム側で付与する
...
- ソート順は新しい受信日時順を基本とする
- ページネーションを提供する
- 画面上のトグルボタンにより自動更新の ON/OFF を切り替えられるようにする
- 自動更新 ON 時は一定間隔で一覧画面を再読み込みする
...
- `GET /settings/mappings` の管理画面から SFTP 転送設定を変更できるようにする
- 設定対象は以下とする
- SFTP接続先IPアドレス
- ユーザ名
- パスワード
- 転送頻度(分)
- 転送先パス
- 転送先パスはファイル名のみ、または任意のリモートパスを指定可能とする
...
- 保存前に簡易バリデーションを実施する
- SFTP 転送設定として、接続先IP、ユーザ名、パスワード、転送頻度、転送先パスを設定できるようにする
